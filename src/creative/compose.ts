...
export async function componer(input: {
  ...
  if (titular || cta) {
    const tamTitular = Math.round(W * 0.075);
    const tamCta = Math.round(W * 0.038);
    ...
    let botones = '';
    if (cta) {
      const anchoBoton = Math.min(anchoUtil, Math.round(cta.length * tamCta * 0.62) + tamCta * 2);
      const altoBoton = Math.round(tamCta * 2.4);
