...
async function post(text) {
  if (!config.linkedin.authorUrn) {
    throw new Error('LINKEDIN_AUTHOR_URN is not set (expected e.g. urn:li:person:XXXX).');
  }
  const body = {
  ...
  const id = data?.id || resHeaders['x-restli-id'];
...
module.exports = { post, check, platform: 'linkedin' };
