...
export async function POST(req: NextRequest) {
  ...
  try {
    const { askWithHistory } = await import('@/src/agent/index');
    ...
    const reply = await askWithHistory(turns, prompt);

    if (userId) {
      await saveMessage({ userId, role: 'user', content: prompt }).catch(() => undefined);
      await saveMessage({ userId, role: 'assistant', content: reply }).catch(() => undefined);
    }
